...
Vec3 Camera::world2camera(const Vec3 &p_w, const SE3 &T_c_w) {
    return T_rect_ * T_c_w * p_w;
}

Vec3 Camera::camera2world(const Vec3 &p_c, const SE3 &T_c_w) {
    return T_c_w.inverse() * T_rect_inv_ * p_c;
}
...
Vec3 Camera::camera2lidar(const Vec3 &p_c){
    //cout << "T_l_c_:" << T_l_c_.log() << " |  p_c : " << p_c.transpose() <<endl;
    return T_rect_inv_ * T_l_c_ * p_c;
}
